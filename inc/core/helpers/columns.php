...
class Columns
{
    ...
    public function register_columns_cpt($columns)
    {
        ...
        foreach ($columns as $key => $value) {
            if ($key == $before) {
                $new_columns['title'] = __('Job', 'revolt-framework');
    ...
    public function populate_custom_columns_cpt($column, $post_id)
    {
        switch ($column) {
            ...
            case 'company':
                $companyMeta = get_post_meta($post_id, 'revolt-job-post-company-field', true);
            ...
            case 'salary':
                $salaryMeta = get_post_meta($post_id, 'revolt-job-post-salary-field', true);
                $salaryMetaOptional = get_post_meta($post_id, 'revolt-job-post-salary-field-optional', true);
