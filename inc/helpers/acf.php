...
namespace Revolt_Framework\Inc\Helpers\ACF;
...
add_filter('acf/validate_value/name=the_job_categories', __NAMESPACE__ . '\only_allow_10_categories', 20, 4);
function only_allow_10_categories($valid, $value, $field, $input)
{
    if (count($value) > 10) {
        $valid = 'Select only up to 10 categories';
    };
    return $valid;
}
...
{
    // update path
    $path = REVOLT_FRAMEWORK_DIR . '/frontend/react-dashboard/src/acf-json';
    // return
    return $path;
}
...
function my_acf_json_load_point($paths)
{
    ...
    unset($paths[0]);
    // append path
    $paths[] = REVOLT_FRAMEWORK_DIR . '/frontend/react-dashboard/src/acf-json';
    // return
    return $paths;
}
